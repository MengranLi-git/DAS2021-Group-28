...
```{r echo=FALSE,eval=TRUE,message=FALSE,warning=FALSE}
 Data %>%
  ...
  adorn_pct_formatting()%>%
  adorn_ns()%>%
  knitr::kable(caption = '\\label{tab:varietyscore} Pass\\% and Fail\\% of each Wine Variety')%>%
...
```{r, echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE,fig.cap="\\label{fig:model1} Odds Ratio Graph Based on GLM Model ", out.width='75%', fig.pos='H'}
plot_model(fit3, show.values = TRUE,
           title = "Odds (price)", show.p = TRUE)
```

```{r, echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE, fig.cap="\\label{fig:predmodel} Probability of Pass for Italy \\& New Zealand ", out.width='75%', fig.pos='H'}
plot_model(fit3,
           type = "pred", title = "",
