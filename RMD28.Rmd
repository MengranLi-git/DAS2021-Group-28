...
So we set all countries except 'New Zealand' and 'Italy' as ‘others’ to make this variable a binary categorical variable.
```{r glm, echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE}
Data <- Data %>% mutate(
  country = case_when(
...
After comparing the AIC values, model 2 was finally used for more in-depth research. 
...
```
The result is less than 1, so there is no overdispersion.

Notice that the coefficients of price, countryNew Zealand are positive, which means that expensive wine are more likely to pass(points is greater than 90). And the all coefficients are significant (p-value of *??*, *??* and *??*), so we qualify the effect of them.
...
The above figure shows that every unit increase in price, the expensive wines' odds of passing are 1.06 times those of the cheap wine. And for other places produced wine's odds of getting more than 90 points are *??* times that of Italy produced wine, while New Zealand produced wine's odds of passing are *??* times that of Italy produced wine.

After establishing the generalised linear model and comparing them, we found that the price is the most significant factor on wine scores. In addition, after analysis, we found that New Zealand produced wine is easier to reach 90 points than other varieties of wine, while the Italy produced wine is harder to pass.
